% l-tuple mSGD algorithm for CGM application (noise mask)

function [xhat, approx_err, resid_err, err_time] = ltuple_mSGD_noise(A, x, y, maxiter, p, ell, alpha)
...
while current_iter < maxiter
    ...
    % Call g function 
    gxk = (1/p^2) * ( Ai' * (Ai * xhat - p * y(i)) ) - ((1-p)/p^2)*(L .* (Ai'*Ai)) * xhat;
