% mSGD algorithm for CGM application

function [xhat, approx_err, resid_err, err_time] = mSGD(A, x, y, maxiter, p, ell, alpha)
...
while current_iter < maxiter
    ...
    % Call g function 
    gxk = (1/p^2) * ( Ai_tilde' * (Ai_tilde * xhat - p * y(i)) ) - ((1-p)/p^2)*((diag(diag(Ai_tilde'*Ai_tilde)))) * xhat;
